...
当前仓库已经不再是单纯工程骨架，而是一个可运行、可登录、可配置、可执行规则、可查看运行历史、支持自动整理与手动整理的可用版本。`v3.3` 在此前规则执行与界面整理能力基础上，进一步补齐了系统设置页中的规则配置备份能力：现已支持导出全部规则为 JSON 备份，并可通过导入备份覆盖恢复当前规则；同时保留了规则弹窗模式卡片宽度对齐、收集归档递归收集/同名判重/`-re` 后缀避让/源文件清理等既有增强。后续仍可继续扩展更细的文件管理能力、日志治理、批量任务编排与部署体验。
