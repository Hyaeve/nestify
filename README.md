...
当前仓库已经不再是单纯工程骨架，而是一个可运行、可登录、可配置、可执行规则、可查看运行历史、支持自动整理与手动整理的可用版本。`v3.2` 进一步完善了规则弹窗中的模式卡片宽度对齐体验，并补齐了收集归档的真实执行语义：支持递归收集控制、同名文件内容判重、异文件自动追加 `-re` 后缀，以及按开关决定是否保留源文件。后续仍可继续扩展更细的文件管理能力、日志治理、批量任务编排与部署体验。
