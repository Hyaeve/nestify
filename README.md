# Nestify

<img src="./Nestify.png" alt="Nestify Logo" width="180" />

Nestify 是一个面向 NAS / 家庭服务器场景的可视化文件整理与规则执行平台，当前已具备完整的前后端界面、SQLite 持久化、规则管理、任务日志、目录浏览、系统设置与基础自动化执行能力。

当前版本为 `v3.4`。

## 主要能力

- 单管理员登录与会话管理
- 仪表盘概览：
  - 总规则数
  - 今日处理量
  - 运行中任务
  - 最近执行摘要
  - 系统资源占用
- 规则管理：
  - 归档规则
    - 打包归档
    - 收集归档
    - 匹配归档：按关键词或扩展名直接分流文件到目标目录，不参与打包
    - 单件归巢：命中裸露文件后自动创建同名目录并转移
    - 嵌套打包 / 扁平归档 / 清理源文件 / 归档校验等模块化开关
  - 净化规则
    - 清理空文件夹
    - 按文件名匹配规则清理文件
    - 繁转简
    - 匹配转换（按自定义规则重命名文件或文件夹）
  - 链路规则
    - 软链模式
    - 硬链模式
    - 文件名黑名单，一行一条，命中即跳过创建链路
- 目录选择器：
  - 支持系统根目录浏览
  - 支持手动输入路径
  - 默认直接展示根目录下一层子目录
- 手动整理：支持浏览目录并对条目进行手动打包
- 任务日志与运行历史
- 系统设置：
  - 日志保留天数
  - 最大日志条数
  - 管理员账号修改

## 当前规则说明

### 1. 归档规则

支持两种模式：

- `package`：按卷或目录打包归档
- `collect`：收集文件到目标目录

支持：

- 实时监听执行
- 定时执行
- 启动后立即执行
- 本地模式 / 兼容模式
- 文件名匹配黑名单：命中后跳过归档处理
- 打包归档附加模块：
  - 扁平归档
  - 归档清单
  - 归档校验
  - 清理源文件
  - 嵌套打包
- 匹配归档：
  - 仅在打包归档模式生效
  - 一行一条规则
  - 支持按扩展名匹配，例如 `mp4`
  - 支持按关键词匹配，例如 `cover`
  - 命中的文件直接转移到目标路径对应目录，不参与 CBZ 打包
- 单件归巢：
  - 仅在打包归档模式生效
  - 只处理监控目录根下的裸露文件
  - 一行一条规则
  - 支持关键词、扩展名或正则匹配文件名
  - 命中后自动整理为 `目标目录/文件主名/原文件名`
- 收集归档附加模块：
  - 递归收集
  - 同名去重（同内容跳过，异内容自动追加 `-re` 后缀）
  - 清理源文件（关闭后改为保留源文件）

### 2. 净化规则

用于清理与转换监控目录下的内容，支持两类模式：

- `cleanup`：
  - 清理空文件夹
  - 按文件名规则清理文件
  - 空文件夹清理白名单：一行一个文件夹全称，仅跳过当前层空目录删除，子目录仍按规则继续清理
- `transform`：
  - 繁体中文转换为简体中文
  - 按自定义规则批量重命名文件和文件夹

文件名规则为一行一条，可写普通文本或正则表达式。转换规则支持自定义文本匹配与替换。

### 3. 链路规则

链路规则已并入“规则管理”页面，与净化规则同级展示。

支持：

- 软链模式
- 硬链模式
- 自定义监控目录
- 自定义链路目录
- 实时监听新文件出现后自动创建链路
- 计划执行全量创建链路
- 文件名黑名单：
  - 一行一条
  - 支持文本或正则
  - 仅匹配文件名
  - 命中后跳过创建链路

默认开启计划执行时，Cron 表达式为：

```text
30 4 * * *
```

即每天 `04:30` 执行一次。

## 日志保留策略

系统设置页已支持日志保留策略保存。

默认值：

- 日志保留天数：`5`
- 最大日志条数：`10000`

运行历史在写入时会自动应用保留策略：

- 超过保留天数的历史记录自动清理
- 超过最大条数时，仅保留最新的记录

## 目录结构

```text
.
├─ backend/                # Go 后端
├─ frontend/               # Vue 3 + Element Plus 前端
├─ config/                 # 配置示例
├─ data/                   # SQLite、运行数据、临时文件
├─ deploy/                 # 部署说明与示例
└─ docs/                   # 架构与设计文档
```

## 技术栈

### 后端

- Go
- SQLite
- 原生 `net/http`
- [`robfig/cron`](backend/go.mod)

### 前端

- Vue 3
- TypeScript
- Vite
- Element Plus
- Pinia

## 已落地的后端能力

- 健康检查接口：`GET /api/v1/health`
- 系统资源接口：`GET /api/v1/system/resource`
- 规则 CRUD
- 规则排序调整
- 规则手动执行预检查与执行
- 运行历史查询与清理
- 管理员账号更新
- 系统设置读取与保存
- 目录浏览与目录校验
- 自动化规则调度与监听
- SQLite 自动迁移（含规则字段增量迁移）

## 已落地的前端页面

- 登录页
- 仪表盘
- 规则管理页
  - 归档规则
  - 净化规则
  - 链路规则
  - 运行历史
- 手动整理页
- 任务日志页
- 系统设置页

## 本地开发

### 启动后端

```bash
cd backend
go run ./cmd/server
```

默认监听：

```text
:8080
```

### 启动前端

```bash
cd frontend
npm install
npm run dev
```

前端开发服务器默认监听 `5173`，并将 `/api` 请求代理到后端。

### 构建检查

后端：

```bash
cd backend
go build ./...
```

前端：

```bash
cd frontend
npm run build
```

## 环境变量

可通过环境变量覆盖运行配置：

- `NESTIFY_HTTP_ADDR`
- `NESTIFY_WEB_DIR`
- `NESTIFY_DB_PATH`
- `NESTIFY_ADMIN_INITIAL_USERNAME`
- `NESTIFY_ADMIN_INITIAL_PASSWORD`
- `NESTIFY_BROWSE_ROOTS`

## 默认管理员初始化

首次启动且数据库中不存在管理员账号时，会自动创建管理员：

- 用户名默认值：`admin`
- 密码默认值：`nestify123`

建议生产环境覆盖：

```bash
NESTIFY_ADMIN_INITIAL_USERNAME=your-admin
NESTIFY_ADMIN_INITIAL_PASSWORD=your-strong-password
```

## 路径浏览根目录

路径选择器默认自动推断可浏览根目录：

- Windows：自动推断可用盘符
- Linux / 容器：默认使用根目录 `/`

如需限制前端只能浏览指定目录，可设置：

```bash
NESTIFY_BROWSE_ROOTS=/library;/archive;/config
```

多个根目录使用分号分隔。

## 容器运行

```bash
docker compose up --build
```

容器模式下：

- 前端构建产物由后端从 `/app/web` 托管
- 默认访问地址：`http://localhost:8080`

### Docker Compose 部署示例

```yaml
services:
  nestify:
    image: ghcr.io/hyaeve/nestify:latest
    container_name: Nestify
    network_mode: bridge
    ports:
      - 8080:8080
    volumes:
      - ./config:/config
      - ./data:/data
      - ./logs:/logs
    environment:
      NESTIFY_HTTP_ADDR: ":8080"
      NESTIFY_CONFIG_PATH: "/config/config.yaml"
      NESTIFY_WEB_DIR: "/app/web"
      NESTIFY_DB_PATH: "/data/app.db"
      NESTIFY_ADMIN_INITIAL_USERNAME: "admin"
      NESTIFY_ADMIN_INITIAL_PASSWORD: "password"
    restart: always
```

说明：

- 将配置文件放置到宿主机 [`config/config.yaml`](config/config.example.yaml)
- 首次启动前建议修改默认管理员账号与密码
- 持久化目录建议保留 [`./data`](data/) 与 [`./logs`](logs/)

## GitHub 自动构建镜像

仓库已包含工作流 [`docker-image.yml`](.github/workflows/docker-image.yml)，可在推送到 GitHub 后自动构建容器镜像。

### 触发方式

- 推送到 `main` / `master`
- 推送标签 `v*`
- 手动触发 GitHub Actions
- Pull Request 时执行构建验证但不推送镜像

### 镜像仓库

默认推送到 GitHub Container Registry：

```text
ghcr.io/<owner>/<repo>
```

### 自动标签

- `latest`
- 分支名标签
- Git 标签
- 提交 SHA 标签

## 说明

当前仓库已经不再是单纯工程骨架，而是一个可运行、可登录、可配置、可执行规则、可查看运行历史、支持自动整理与手动整理的可用版本。`v3.4` 在 `v3.3` 的规则备份恢复与归档增强基础上，进一步补齐了净化规则中的清理模式白名单能力：现已支持为“清理空文件夹”配置一行一个的文件夹全称白名单，命中的目录仅跳过当前层空目录删除，其下未命中白名单的子目录仍会继续递归清理；同时保留规则配置备份、递归收集、同名判重、`-re` 后缀避让、源文件清理与界面模式卡片等既有能力。
