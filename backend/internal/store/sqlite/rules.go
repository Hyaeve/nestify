	...
	rows, err := s.db.Query(`
		SELECT id, sort_order, name, description, enabled, monitor_enabled, compatibility_mode, archive_mode, rule_type, link_mode, run_mode,
		       source_dir, target_dir, watch_debounce_ms, cron_expression, run_on_start,
			       options_json, option_values_json, package_options_json, collect_options_json, filters_json, metadata_filters_json, whitelist_json, match_filters_json, nest_filters_json, transform_rules_json, transform_filters_json,
...
func (s *Store) CreateRule(input model.CreateRuleInput) (*model.Rule, error) {
	...
	`,
		...
		marshalStringList(input.MetadataFilters),
...
func (s *Store) ReplaceRules(items []model.Rule) error {
	...
	stmt, err := tx.Prepare(`
		INSERT INTO rules (
			sort_order, name, description, enabled, monitor_enabled, compatibility_mode, archive_mode, rule_type, link_mode, run_mode,
			source_dir, target_dir, watch_debounce_ms, cron_expression, run_on_start,
			options_json, package_options_json, collect_options_json, filters_json, metadata_filters_json, whitelist_json, match_filters_json, nest_filters_json, transform_rules_json, transform_filters_json,
			last_run_status, last_success_count, last_skip_count, last_failure_count,
			created_at, updated_at
		) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`)
	...
	for index, item := range items {
		...
		if _, err := stmt.Exec(
			...
			defaultRuleJSONText(item.MetadataFiltersJSON, `[]`),
