...
const historyItems = ref<Array<{ rule_name: string; summary: string; status: string; success_count: number; skip_count: number; failure_count: number; started_at: string }>>([])
...
async function prepareExecution(ruleID: number) { loading.value = true; errorMessage.value = ''; try { const response = await prepareRuleExecution(ruleID, 'once'); const run = response.data?.run; latestPreparedSummary.value = response.data?.prepared?.summary ?? ''; if (run) { const logsResponse = await fetchRunLogs(run.id); const items = logsResponse.data?.items ?? []; historyItems.value.unshift({ rule_name: run.rule_name || '未知规则', summary: items.map((item) => item.message).join(' | ') || latestPreparedSummary.value, status: run.status === 'succeeded' ? 'success' : run.status === 'failed' ? 'failed' : 'skip', success_count: run.success_count, skip_count: run.skip_count, failure_count: run.failure_count, started_at: run.started_at }) } ElMessage.success('规则执行完成'); activeTab.value = 'history'; await loadRules() } catch (error) { errorMessage.value = error instanceof Error ? error.message : '规则执行失败' } finally { loading.value = false } }
