    ...
    <el-dialog v-model="ruleDialogVisible" title="添加规则" width="860px" destroy-on-close class="naming-rule-dialog">
      <div class="rule-dialog-shell">
        ...
        <section class="rule-config-panel">
          ...
          <div v-else-if="activeRuleCategory === 'pad'" class="insert-config">
            <el-form label-position="top">
              <div class="insert-config__box">
              ...
              <el-form-item label="填充字段">
                <el-input v-model="padRuleDraft.content" size="small" class="insert-text-input" placeholder="例如：2026." />
              </el-form-item>
...
const padPositionOptions = [
  { label: '之前', value: 'before' },
  { label: '之后', value: 'after' },
]

const padRuleDraft = reactive<PadRuleConfig>({
  content: '',
  index: 1,
  position: 'before',
  smart: false,
...
function handlePreview() {
  ...
  resultItems.value = items.map((item) => ({ ...item, nextName: applyAddedRules(item) }))
...
function saveRuleDraft() {
  if (activeRuleCategory.value === 'pad' && !padRuleDraft.content) {
...
function applyPadRule(name: string, kind: SourceItemKind, config: PadRuleConfig) {
  ...
  const insertIndex = Math.min(characterIndex + (config.position === 'after' ? 1 : 0), characters.length)

  if (config.smart && characters.slice(insertIndex, insertIndex + contentCharacters.length).join('') === config.content) {
    return name
  }

  return `${characters.slice(0, insertIndex).join('')}${config.content}${characters.slice(insertIndex).join('')}${extension}`
}
...
.pane-count {
  ...
  border-radius: 999px;
  color: #4a6b5f;
  background: rgba(127, 178, 162, 0.1);
  font-size: 12px;
  font-weight: 700;
}
...
.result-item:hover {
  border-color: rgba(127, 178, 162, 0.32);
...
.folder-child-item:hover {
  border-color: rgba(127, 178, 162, 0.28);
...
.rail-button--preview .line-icon,
.rail-button--rename .line-icon {
  width: 30px;
  height: 30px;
  stroke-width: 1.85;
  opacity: 0.78;
...
.rail-arrow {
  ...
  color: #5b7a6e;
  border-radius: 24px;
  background: linear-gradient(135deg, rgba(127, 178, 162, 0.16), rgba(127, 178, 162, 0.08));
  border: 1px solid transparent;
}
...
.rule-nav-item.is-active {
  color: #5b7a6e;
  background: rgba(127, 178, 162, 0.11);
}

.rule-nav-item.is-active {
  box-shadow: inset 3px 0 0 #5b7a6e;
}
...
.added-rule-item__order {
  ...
  font-weight: 800;
  border-radius: 999px;
  background: rgba(127, 178, 162, 0.12);
}
...
.rule-set-card.is-active {
  color: #4a6b5f;
  border-color: rgba(127, 178, 162, 0.26);
